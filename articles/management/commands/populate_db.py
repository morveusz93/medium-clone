...
class Command(BaseCommand):
    ...
    def create_tags(self):
        tag_names = ["Recipies", "Tech", "Movies", "Sport", "Music", "Animals", "Culture"]
        tags = []
        for i in tag_names:
            tag = Tag(name=i)
            try:
    ...
    def create_articles(self, authors, tags):
        lorem = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis nec orci vulputate, gravida mi " \
        ...
        articles = []
        for i in range(100):
            article = Article(title="title " + str(i), author=random.choice(authors), content=lorem)
            article.save()
